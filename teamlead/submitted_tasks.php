...
$submitted_tasks_query = "
    ...
    WHERE t.assigned_by = $team_lead_id AND t.status = 'submitted'
    ORDER BY t.submitted_date DESC
";

$submitted_tasks_result = mysqli_query($conn, $submitted_tasks_query);
?>
...
<head>
    ...
    <title><?php echo $company_name; ?> - Submitted Tasks</title>
...
<body>
    ...
    <div class="sidebar">
        ...
        <div class="sidebar-menu">
            ...
            <a href="submitted_tasks.php" class="nav-link active">
                <i class="fas fa-paper-plane"></i>Submitted Tasks
                <?php 
                $count = mysqli_num_rows($submitted_tasks_result);
                if ($count > 0): ?>
                <span class="badge bg-info float-end"><?php echo $count; ?></span>
                <?php endif; ?>
            </a>
    ...
    <div class="main-content">
        ...
        <nav class="navbar-top">
            <div class="container-fluid">
                <div class="d-flex justify-content-between align-items-center w-100">
                    <div>
                        <h4 class="mb-0">Submitted Tasks</h4>
                        <small class="text-muted">Review work submitted by your interns</small>
                    </div>
                </div>
            </div>
        </nav>
        ...
        <div class="card">
            <div class="card-header d-flex justify-content-between align-items-center">
                <h6 class="mb-0"><i class="fas fa-paper-plane me-2"></i>Recent Submissions</h6>
                <span class="badge bg-primary"><?php echo mysqli_num_rows($submitted_tasks_result); ?> pending review</span>
            </div>
            <div class="card-body">
                <?php if (mysqli_num_rows($submitted_tasks_result) > 0): ?>
                    <div class="table-responsive">
                        <table class="table table-hover">
                            ...
                            <tbody>
                                <?php while($task = mysqli_fetch_assoc($submitted_tasks_result)): ?>
                                    <tr class="task-row" onclick="window.location.href='view_task.php?id=<?php echo $task['id']; ?>'">
                                        ...
                                        <td>
                                            ...
                                            <br>
                                            <small class="text-muted"><?php echo date('h:i A', strtotime($task['submitted_date'])); ?></small>
                                        </td>
                                        <td>
                                            <?php echo date('M d, Y', strtotime($task['deadline'])); ?>
                                            <?php 
                                            if (strtotime($task['submitted_date']) > strtotime($task['deadline'])) {
                                                echo '<br><span class="badge bg-danger">Late Submission</span>';
                                            }
                                            ?>
                                        </td>
                                        <td onclick="event.stopPropagation();">
                                            <?php if ($task['submitted_file']): ?>
                                                <a href="../assets/uploads/tasks/<?php echo $task['submitted_file']; ?>" class="btn btn-sm btn-outline-info" download>
                                        ...
                                        <td onclick="event.stopPropagation();">
                                            <div class="btn-group" role="group">
                                                <a href="review_task.php?id=<?php echo $task['id']; ?>" class="btn btn-sm btn-outline-info" title="Review Task">
                                                    <i class="fas fa-clipboard-check"></i>
                                                </a>
                                                <a href="edit_task.php?id=<?php echo $task['id']; ?>" class="btn btn-sm btn-outline-warning" title="Edit Task">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <a href="delete_task.php?id=<?php echo $task['id']; ?>" class="btn btn-sm btn-outline-danger" title="Delete Task" onclick="return confirm('Are you sure you want to delete this task?')">
                ...
                <?php else: ?>
                    <div class="text-center py-5">
                        ...
                        <p class="text-muted mb-4">You have no new task submissions to review at this time.</p>
